...
router.get('/signup-complete', (req, res) => {
    ...
    // Render signup-complete page with user data
...
router.post('/signup-complete', async (req, res) => {
    try {
        ...
        if (!password || !nama_ortu || !no_hp_ortu || !nama || !asal_sekolah || !phone) {
            return res.redirect('/signup-complete?message=All fields are required');
        }
        ...
        await User.update({
            ...
            paket: paket || 'tryout',
            jenjang: jenjang || 'SMA',
            program: program || 'gg-lite'
        }, {
            where: { id: req.user.id }
        });
        
        // Update session to reflect the new user data
        const updatedUser = await User.findByPk(req.user.id);
        req.login(updatedUser, (err) => {
            if (err) {
                console.error('Error updating session:', err);
                return res.redirect('/signup-complete?message=Error updating session');
            }
        ...
        res.redirect('/signup-complete?message=Internal server error');
    }
});
